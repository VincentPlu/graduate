    ...
    leaflet.mainTemplate = function () {
        ...
        this.init = function (map,params) {
            ...
            _map.doDuringRead=function(response,data) {
                ...
                if(data.readDataOnly){
                    ...
                    _tree = new leaflet.mainTree();
                    _tree.init(treeOptions)

                    //websocket相关
                    _self.initWebSocket({webSocketUrl:response.configVo.webSocketUrl});
                    _map.setWebSocket(_mainWebsocket);
        ...
        this.setIcons=function(){
            ...
            map.ICONS.setTemplate("alertInfo",function(params){
                return new L.divIcon({
                    targetId:"info_"+params.devId+"_"+params.targetId,
                    html:''
        ...
        this.dataFormat=function(markers,center){
            ...
            for(var i in markers){
                ...
                if(markerType=="area"){
                    ...
                    var iconUrl = "/template/detection/img/device_online_small.png";
                    if(markers[i].deviceType=="product"){
                        if(markers[i].productTypeCode=="PRZ") {//区域产品
                            var iconUrl = "/template/detection/img/prz_online_small.png";
                            if (markers[i].status == "0") {
                                iconUrl = "/template/detection/img/prz_offline_small.png";
                            } else if (markers[i].status == "2") {
                                iconUrl = "/template/detection/img/prz_error_small.png";
                            }
                        }else{
                            if (markers[i].status == "0") {
                    ...
                    }else if(markers[i].deviceType=="device"){
                        // if(markers[i].productId==''||markers[i].productId==null){
                        ...
                        //     }
                        // }
                        iconUrl = "/template/detection/img/dev.png";
                    }
            ...
            ws.init({
                ...
                onmessage:function (evt) {
                    if(evt.data){
                        ...
                        if(mapId){
                            _map.LAYERS.switchLayer("MapInfo_"+mapId,_map.getMap());
                        }
                    }
