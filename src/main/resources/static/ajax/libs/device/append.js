...
var appendDevice = function (response) {
    ...
    for (var i = 0; i < response.rows.length; i++) {
        if (response.rows[i].deviceTypeCode.indexOf("EV") != -1 || response.rows[i].deviceTypeCode.indexOf("ER") != -1) {
        ...
        return {
            addParams: function () {
                ...
                if (data.inputType == 'input') {//如果参数为input
                    ...
                    addParam += "<select class=\"form-control\" name='" + data.attributeCode + "'" + isRequired.req + " " + isEdit.isEdit + ">";
