...
(function ($) {
    $.extend({
    	...
    	modal: {
			...
			// 弹出层指定宽度
			newOpen: function (title, url, width, height, callback1,callback2) {
				//如果是移动端，就使用自适应大小弹窗
				if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
					width = 'auto';
					height = 'auto';
				...
				if ($.common.isEmpty(callback1)) {
					callback1 = function(index, layero) {
						var iframeWin = layero.find('iframe')[0];
						iframeWin.contentWindow.submitHandler(index, layero);
					}
				}

				if ($.common.isEmpty(callback2)) {
					callback2 = function (index, layero) {
						var apply = layero.find('iframe')[0];
						apply.contentWindow.apply(index, layero);
						return false;
					}
				}
            ...
            // 弹出系统信息
            openSysInfo: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
			...
			// 批量删除Strategy信息
			removeStrategyAll: function (callback) {
				var data =$('#bootstrap-table').bootstrapTable('getSelections');
				var strategyCodes = [];
				for (var i in data) {
					strategyCodes.push(data[i].strategyCode);
				}
				var rows = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
				if (rows.length == 0) {
					$.modal.alertWarning("请至少选择一条记录");
				...
				$.modal.confirm("确认要删除选中的" + rows.length + "条数据吗?", function () {
					var url = $.table._option.removeUrl;
					var data = {"ids": rows.join(),"strategyCodes":strategyCodes.join()};
					$.operate.submit(url, "post", "json", data, callback);
				});
			},
			// 清空信息
            ...
            add: function(id,width,height) {
                $.modal.open("添加" + $.table._option.modalName, $.operate.addUrl(id),width,height);
            },
			//添加有应用的信息
			applyEdit:function(id,callback2){
				$.modal.newOpen("添加" + $.table._option.modalName, $.operate.addUrl(id),null,null,null,callback2);
			},
            ...
            edit: function(id,width,height) {
            	if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
            	    ...
            	    $.modal.open("修改" + $.table._option.modalName, $.operate.editUrl(id),width,height);
            	}
            },
			//编辑设备
			editDevice: function(deviceId,typeId) {
				var url = $.table._option.updateUrl.replace("{id}", deviceId).replace("{typeId}",typeId);
				$.modal.newOpen("修改" + $.table._option.modalName, url);
			},

            editProduct:function(deviceId,typeId){
                var url = $.table._option.updateUrl.replace("{id}", deviceId).replace("{typeId}",typeId);
                $.modal.open("修改" + $.table._option.modalName, url);
            },

			//编辑应用设备
			editApplyDevice: function (deviceId, typeId) {
				var url = $.table._option.updateUrl.replace("{id}", deviceId).replace("{typeId}",typeId);
				$.modal.open("修改" + $.table._option.modalName, url);
			},
			...
			saveType: function(url, data, callback) {
				var config = {
					url: url,
					type: "post",
					dataType: "json",
